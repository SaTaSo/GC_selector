...
public class ResultsService extends Profiler {
    ...
    public void fetchMatrix(UserInputs ui, Statistics st) {
        userInputs = ui;
        statistics = st;
        System.out.println("Maximum heao used by the user's application: "+ statistics.getMaxHeapUsage());
        ...
        if (st.getIsCpuIntensive()) {
            try {
                ...
                matrix = (Map<String, Double>) matrixField.get(0);
    ...
    public void calculateResults(Map<String, Double> matrix, Double weightThroughput, Double weightPause) {
        for (String gc : gcs) {
            ...
            String p = gc + "_pause";
            Double x = (matrix.get(t.toLowerCase()) * weightThroughput) + (matrix.get(p.toLowerCase()) * weightPause);
            results.put(gc, x);
        }
    }
    ...
    public String createExecutionCommand() {
        ...
        String gc = finalOrderedResults.get(0).getGc().equals("ZGC") ? "ZGC" : finalOrderedResults.get(0).getGc() + "GC";
        String appToRun = "";
        if (statistics.getPid() != null) {
            try {
                ProcessBuilder processBuilder = new ProcessBuilder();
                processBuilder.command("bash", "-c", "jcmd | grep \":" + statistics.getPid() + "\" | awk '{print $2}' ");
                Process getApp = null;
